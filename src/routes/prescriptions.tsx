...
function PrescriptionsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 pb-24 md:pb-8">
      <div className="mb-5">
        <div className="text-[12px] font-semibold uppercase tracking-[0.08em] text-primary">Pharmacy Services</div>
        <h1 className="mt-1 text-2xl font-bold tracking-tight text-[#111827] md:text-3xl">Upload Your Prescription</h1>
        <p className="mt-1 text-sm text-[#6B7280]">A registered Plus2 pharmacist will review your script within 2 hours (Mon–Sat 8am–6pm).</p>
      </div>
      <UploadWizard onSubmit={addPrescription} />
      {prescriptions.length > 0 && <RecentScripts list={prescriptions.slice(0, 3)} />}
    </div>
  );
}

function UploadWizard({ onSubmit }: { onSubmit: ReturnType<typeof useAuth.getState>["addPrescription"] }) {
  ...
  const submit = () => {
    const id = onSubmit({
      ...
      files: validFiles.map((f) => ({ name: f.file.name, size: f.file.size, type: f.file.type, dataUrl: f.preview })),
      forSelf,
  ...
  return (
    <div className="rounded-lg border border-[#E5E7EB] bg-white shadow-sm">
      ...
      <div className="p-5 md:p-6">
        {step === 1 && <Step1Files files={files} setFiles={setFiles} />}
        {step === 2 && (
          <Step2Patient
            ...
            scriptDate={scriptDate} setScriptDate={setScriptDate}
            isRepeat={isRepeat} setIsRepeat={setIsRepeat}
            repeatsLeft={repeatsLeft} setRepeatsLeft={setRepeatsLeft}
            notes={notes} setNotes={setNotes}
          />
        )}
        {step === 3 && <Step3Delivery delivery={delivery} setDelivery={setDelivery} />}
        {step === 4 && (
          <Step4Review
            files={validFiles} forSelf={forSelf} patient={patient} relationship={relationship}
            doctor={doctor} scriptDate={scriptDate} isRepeat={isRepeat} repeatsLeft={repeatsLeft}
            delivery={delivery} confirm={confirm} setConfirm={setConfirm}
          />
        )}
        {step === 5 && refId && <Step5Success refId={refId} />}
      </div>
      {step < 5 && (
        <div className="sticky bottom-0 z-10 flex items-center justify-between gap-3 border-t border-[#E5E7EB] bg-white p-4 md:rounded-b-lg">
          {step > 1 ? (
            <button onClick={goBack} className="inline-flex items-center gap-1.5 rounded-md border border-[#E5E7EB] bg-white px-4 py-2.5 text-sm font-semibold text-[#374151] hover:bg-[#F9FAFB]">
          ...
          {step < 4 && (
            ...
            >
              {step === 1 ? "Next: Patient Details" : step === 2 ? "Next: Delivery" : "Next: Review"}
            </button>
          )}
          {step === 4 && (
            <button disabled={!canSubmit} onClick={submit} className="flex-1 rounded-md bg-primary px-5 py-3 text-sm font-semibold text-white hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-50 md:flex-initial md:px-8">
              Submit Prescription
            </button>
          )}
        </div>
      ...
      {labels.map((l, i) => {
        ...
        return (
          <div key={l} className="flex flex-1 items-center gap-2">
            <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] font-bold ${done ? "bg-primary text-white" : active ? "bg-primary text-white" : "bg-[#E5E7EB] text-[#6B7280]"}`}>
              {done ? <CheckCircle2 className="h-4 w-4" /> : idx}
            </div>
            <span className={`hidden text-[11px] font-semibold uppercase tracking-wide md:inline ${active ? "text-primary" : done ? "text-[#374151]" : "text-[#9CA3AF]"}`}>{l}</span>
            {idx < labels.length && <div className={`h-px flex-1 ${done ? "bg-primary" : "bg-[#E5E7EB]"}`} />}
          </div>
        );
      })}
    </div>
...
// ---------- Step 1: Files ----------
function Step1Files({ files, setFiles }: { files: LocalFile[]; setFiles: (f: LocalFile[]) => void }) {
  const cameraRef = useRef<HTMLInputElement>(null);
  const galleryRef = useRef<HTMLInputElement>(null);
  const cloudRef = useRef<HTMLInputElement>(null);

  const hasHeic = useMemo(() => files.some((f) => /heic|heif/i.test(f.file.type) || /\.(heic|heif)$/i.test(f.file.name)), [files]);
  ...
  const handleFiles = (list: FileList | null) => {
    ...
    for (const f of incoming) {
      ...
      const error = f.size > MAX_BYTES ? "File too large — max 10MB" : undefined;
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-[#111827]">Add your script</h2>
      ...
      {hasHeic && (
        <div className="flex items-start gap-2 rounded-md border border-[#FDE68A] bg-[#FEFCE8] p-3 text-xs text-[#854D0E]">
          ...
          <span>iPhone HEIC format detected — will be converted automatically by our system.</span>
        </div>
      )}
      ...
      {files.length > 0 && (
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {files.map((f) => (
              <div key={f.id} className={`relative aspect-square overflow-hidden rounded-lg border bg-[#F9FAFB] ${f.error ? "border-[#DC2626]" : "border-[#E5E7EB]"}`}>
                {f.preview ? (
                ...
                <button onClick={() => remove(f.id)} aria-label="Remove" className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-white text-[#374151] shadow hover:bg-[#FEE2E2] hover:text-[#DC2626]">
                  <X className="h-3.5 w-3.5" />
                </button>
                <span className="absolute bottom-2 left-2 rounded-[3px] bg-black/50 px-1.5 py-0.5 text-[10px] font-medium text-white">{(f.file.size / 1024 / 1024).toFixed(1)} MB</span>
                {f.error && <div className="absolute inset-x-0 bottom-0 bg-[#DC2626] px-2 py-1 text-center text-[10px] font-semibold text-white">{f.error}</div>}
              </div>
            ))}
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-[#6B7280]">
            <button onClick={() => galleryRef.current?.click()} className="inline-flex items-center gap-1.5 rounded-md border border-primary px-3 py-1.5 text-xs font-semibold text-primary hover:bg-[#F0F9F4] disabled:opacity-50" disabled={files.length >= MAX_FILES}>
              <Plus className="h-3.5 w-3.5" /> Add Another Script
            </button>
            <span>{files.length} file{files.length !== 1 ? "s" : ""} selected (max {MAX_FILES}) · Total: {totalMB} MB</span>
          </div>
        </div>
      )}

      <p className="flex items-center gap-1.5 text-[11px] text-[#6B7280]"><ShieldCheck className="h-3 w-3 text-primary" /> Files are encrypted and only viewed by a registered pharmacist.</p>
    </div>
  );
}

function MethodCard({ icon: Icon, title, subtitle, onClick }: { icon: typeof Camera; title: string; subtitle: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="flex w-full items-center gap-4 rounded-lg border border-[#E5E7EB] bg-white px-5 py-4 text-left transition hover:border-primary hover:bg-[#F9FAFB]" style={{ minHeight: 80 }}>
...
function Step2Patient(props: {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Doctor / Prescriber name (optional)" value={doctor} onChange={(e) => setDoctor(e.target.value)} />
        <Field label="Date on prescription" type="date" value={scriptDate} onChange={(e) => setScriptDate(e.target.value)} required />
      </div>

      <div>
        <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-[#374151]">Is this a repeat prescription?</label>
        <div className="grid grid-cols-2 gap-3">
        ...
        {isRepeat && (
          <div className="mt-3">
            <Field label="How many repeats remaining?" type="number" min={1} max={12} value={repeatsLeft} onChange={(e) => setRepeatsLeft(Number(e.target.value) || 1)} />
          </div>
        )}
      </div>
      ...
      <label className="block">
        ...
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} maxLength={300} placeholder="Any specific instructions, preferred generic brands, or questions for the pharmacist..." className="w-full rounded-md border border-[#D1D5DB] bg-white px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
...
function ToggleBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} className={`rounded-md border px-4 py-3 text-sm font-semibold transition ${active ? "border-primary bg-[#F0F9F4] text-primary" : "border-[#E5E7EB] bg-white text-[#374151] hover:bg-[#F9FAFB]"}`}>{children}</button>
  );
}
...
// ---------- Step 3: Delivery ----------
function Step3Delivery({ delivery, setDelivery }: { delivery: "delivery" | "collect"; setDelivery: (v: "delivery" | "collect") => void }) {
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-[#111827]">Delivery Method</h2>
        <p className="text-sm text-[#6B7280]">Choose how you'd like to receive your medication once dispensed.</p>
      </div>
      <div className="grid gap-3 md:grid-cols-2">
        <DeliveryOption active={delivery === "delivery"} onClick={() => setDelivery("delivery")} icon={Truck} title="Home Delivery" subtitle="Delivered to your door within 24 hours · Free over US$50" />
        <DeliveryOption active={delivery === "collect"} onClick={() => setDelivery("collect")} icon={Store} title="Collect in-store" subtitle="Ready for collection at your nearest Plus2 branch" />
      </div>
    </div>
  );
}

function DeliveryOption({ active, onClick, icon: Icon, title, subtitle }: { active: boolean; onClick: () => void; icon: typeof Truck; title: string; subtitle: string }) {
  return (
    <button type="button" onClick={onClick} className={`flex items-start gap-3 rounded-lg border p-4 text-left transition ${active ? "border-primary bg-[#F0F9F4]" : "border-[#E5E7EB] bg-white hover:bg-[#F9FAFB]"}`}>
      <span className={`flex h-10 w-10 items-center justify-center rounded-lg ${active ? "bg-primary text-white" : "bg-[#F0F9F4] text-primary"}`}>
...
function Step4Review(props: {
  files: LocalFile[]; forSelf: boolean; patient: string; relationship: string;
  doctor: string; scriptDate: string; isRepeat: boolean; repeatsLeft: number;
  delivery: "delivery" | "collect"; confirm: boolean; setConfirm: (v: boolean) => void;
}) {
  const { files, forSelf, patient, relationship, doctor, scriptDate, isRepeat, repeatsLeft, delivery, confirm, setConfirm } = props;
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-[#111827]">Review & Submit</h2>
        <p className="text-sm text-[#6B7280]">Confirm the details below before submitting to our pharmacist.</p>
      </div>
      ...
      <Section title="Delivery">
        <p className="text-sm text-[#374151]">{delivery === "delivery" ? "Home Delivery (to your default address)" : "Collect from your nearest Plus2 branch"}</p>
      </Section>

      <div className="flex items-start gap-2 rounded-md border border-[#FDE68A] bg-[#FEF3C7] p-3 text-xs text-[#854D0E]">
        <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
        <span>Important: Ensure your prescription is valid, dated, and signed by a registered healthcare professional. Submitting a forged script is a criminal offence.</span>
      </div>
      ...
      <label className="flex cursor-pointer items-start gap-2 rounded-md border border-[#E5E7EB] bg-[#F9FAFB] p-3 text-sm">
        ...
        <span className="text-[#374151]">I confirm this is a genuine, valid prescription.</span>
      </label>
    </div>
  );
}
...
function DL({ items }: { items: [string, string][] }) {
  return (
    <dl className="grid grid-cols-1 gap-1 text-sm sm:grid-cols-2">
      {items.map(([k, v]) => (
        <div key={k} className="flex gap-2"><dt className="text-[#6B7280]">{k}:</dt><dd className="font-medium text-[#111827]">{v}</dd></div>
      ))}
    </dl>
  );
}
...
// ---------- Step 5: Success ----------
function Step5Success({ refId }: { refId: string }) {
  const stages = ["Submitted", "Under Review", "Approved", "Being Prepared", "Out for Delivery", "Delivered"];
  return (
    <div className="space-y-6 text-center">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-[#6B7280]">Reference: <span className="font-mono font-semibold text-[#111827]">{refId}</span></p>
      </div>
      <p className="mx-auto max-w-md text-sm text-[#374151]">Our pharmacist will review your script within 2 hours during operating hours (Mon–Sat 8am–6pm).</p>

      <div className="overflow-x-auto">
        <ol className="mx-auto flex min-w-[600px] items-center gap-1 px-4">
          {stages.map((s, i) => (
            <li key={s} className="flex flex-1 items-center gap-1">
              <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${i === 0 ? "bg-primary text-white" : "bg-[#E5E7EB] text-[#6B7280]"}`}>{i + 1}</div>
              <span className={`text-[10px] font-medium ${i === 0 ? "text-primary" : "text-[#9CA3AF]"}`}>{s}</span>
      ...
      <p className="text-xs text-[#6B7280]">You'll receive SMS and email updates at each stage.</p>

      <div className="flex flex-col gap-2 sm:flex-row sm:justify-center">
        <Link to="/track" className="rounded-md bg-primary px-6 py-3 text-sm font-semibold text-white hover:bg-primary-dark">Track This Order</Link>
        <button onClick={() => window.location.reload()} className="rounded-md border border-primary px-6 py-3 text-sm font-semibold text-primary hover:bg-[#F0F9F4]">Upload Another Script</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="mt-8 rounded-lg border border-[#E5E7EB] bg-white shadow-sm">
      <div className="border-b border-[#E5E7EB] px-5 py-4">
        <h2 className="text-base font-bold text-[#111827]">Recent Scripts</h2>
      </div>
      <ul className="divide-y divide-[#E5E7EB]">
        {list.map((p) => (
          <li key={p.id} className="flex items-center gap-3 px-5 py-3">
            ...
            <span className="rounded-full bg-[#F0F9F4] px-2 py-0.5 text-[11px] font-semibold text-primary">{p.status}</span>
          </li>
        ))}
      </ul>
    </div>
